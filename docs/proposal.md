...
- The Picking flow this sits alongside **is drawn** -- 66 screens under _Collect products_, including collecting, editing amounts and stock, selecting a division and choosing a next step. An earlier version of this proposal said it was largely undrawn; that was our error in reading the file, which files the picking flow under a section named `09. N/A`.
- A **load-carrier flow already exists** in _Adding products to orders_: cart overview, select a cart, view contents, and verify load carrier contents (_Inhoud ladingdrager_). It answers "what is on this carrier" with a **scrolling list and a checkbox**, not a spatial map -- so the position map described above is a deliberate departure from an established pattern, and should be justified as one. See ADR 0004.
